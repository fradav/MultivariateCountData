...
[@Dempster_1977] to maximize the likelihood. The EM algorithm consists, starts from an initial
...
`ALGORITHM Stepwise`\
$\quad$`BEGIN`\
$\quad$$\quad$`initialize` $X^C$\
$\quad$$\quad$`WHILE` $X^C$ `changes:`\
$\quad$$\quad$$\quad$`- for all variable `$X_j$` which are not in `$X^C$\
$\quad$$\quad$$\quad$$\quad$`- estimate `${\cal M}_1$` on `$X^C \cup X_j$` and select the best `$G$\
$\quad$$\quad$$\quad$$\quad$`- estimate `${\cal M}_2$` with the model for `$X^C$` (with G selected at the previous step)`\
$\quad$$\quad$$\quad$$\quad$`and a Poisson regression for `$X_j$` given `$X^C$\
$\quad$$\quad$$\quad$$\quad$`- add `$X_j$` in `$X^C$` if `$BIC_{{\cal M}_1}>BIC_{{\cal M}_2}$\
$\quad$$\quad$$\quad$`- for each `$X_j$` in `$X^C$\
$\quad$$\quad$$\quad$$\quad$`- estimate `${\cal M}_2$` on `$X^C\setminus X_j$`, select the best `$G$\
$\quad$$\quad$$\quad$$\quad$`and use a Poisson regression for `$X_j$` given `$X^C\setminus X_j$\
$\quad$$\quad$$\quad$$\quad$`- estimate `${\cal M}_1$` on `$X^C$` (with G selected at the previous step)`\
$\quad$$\quad$$\quad$$\quad$`- remove `$X_j$` from `$X^C$` if `$BIC_{{\cal M}_2}>BIC_{{\cal M}_1}$\
$\quad$$\quad$$\quad$`- test for infinite loop`\
$\quad$$\quad$`ENDWHILE`\
$\quad$`return `$X^C$` and `${\cal M}_1$` estimate`\
$\quad$`END`
...
par(mfrow = c(1, 3))
par(mar = c(6, 2, 2, 2))
ARI=matrix(c(ARI1[,1]-ARI1[,3],ARI1[,2]-ARI1[,3]),ncol=2,byrow=FALSE)
colnames(ARI)<-c("Selected var. - True var.","All var. - True var.")
boxplot(ARI, cex.axis=0.7,ylim=c(-.5,.2),las = 2,main='Scenario 1',xlab='');abline(h=0,col=2,las = 2)
ARI=matrix(c(ARI2[,1]-ARI2[,3],ARI2[,2]-ARI2[,3]),ncol=2,byrow=FALSE)
colnames(ARI)<-c("Selected var. - True var.","All var. - True var.")
boxplot(ARI, cex.axis=0.7,ylim=c(-.5,.2),las = 2,main='Scenario 21',xlab='');abline(h=0,col=2,las = 2)
ARI=matrix(c(ARI3[,1]-ARI3[,3],ARI3[,2]-ARI3[,3]),ncol=2,byrow=FALSE)
colnames(ARI)<-c("Selected var. - True var.","All var. - True var.")
boxplot(ARI, cex.axis=0.7,ylim=c(-.5,.2),las = 2,main='Scenario 3',xlab='');abline(h=0,col=2,las = 2)
par(mfrow = c(1, 1))
```
...
par(mfrow = c(1, 3))
par(mar = c(6, 2, 2, 2))
hist(ARI1[, 1] - ARI1[, 2], xlab = 'ARI differences',main='Scenario 1',xlim=c(-.1,.5))
abline(v=0,col=2)
hist(ARI2[, 1] - ARI2[, 2], xlab = 'ARI differences',main='Scenario 2',xlim=c(-.1,.5))
abline(v=0,col=2)
hist(ARI3[, 1] - ARI3[, 2], xlab = 'ARI differences',main='Scenario 3',xlim=c(-.1,.5))
abline(v=0,col=2)
par(mfrow = c(1, 1))
```
...
{
  zmat <- matrix(NA, P, length(yvec))
  Pchosen <- length(fit$jchosen)
  for (p in 1:Pchosen)
  {
    zmat[fit$jchosen[p], ] <- dpois(yvec, fit$bestfit$lambda[g, p])
  }
  rownames(zmat) <- 1:P
  colnames(zmat) <- yvec
  image(xvec, yvec, zmat, xlab = "Hour", ylab = "Laps", main = paste("Cluster ", g, sep = "", collapse = ""))
  matplot(t(x[fit$bestfit$classification == g, ]), xlim = xlim, ylim = ylim, type = "l", add = TRUE, col = "lightgray", lty = 1)
}
par(mfrow = c(1, 1))
```
...
The code for the proposed approach is available as an R package at [https://github.com/JuJacques/MultivariateCountData](https://github.com/JuJacques/MultivariateCountData). 
